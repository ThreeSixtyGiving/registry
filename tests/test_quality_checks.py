...
def test_currency_check_gbp_only():
    ...
    assert c.description == "Contains information about grants made in currencies other than british pounds £."
...
def test_grant_programme_check_true():
    c = GrantProgrammeCheck(RegistryFile({
        'datagetter_coverage': {
    ...
    assert c.description == "This file includes details of the grant programmes that each grant is part of."
...
def test_grant_programme_check_false():
    ...
    assert c.description == "This file does not include details of the grant programmes that each grant is part of."


def test_basic_details_check_true():
    c = BasicDetailsCheck(RegistryFile({
        'datagetter_coverage': {
            '/grants/grantProgramme': {}
        }
    }))
    ...
    assert c.description == "This file contains the basic fields required to meet the 360Giving Data Standard."
...
def test_organisation_identifiers_check_all_internal():
    c = OrganisationIdentifiersCheck(RegistryFile({
        'datagetter_aggregates': {
