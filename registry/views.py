...
def inject_footer_menu():
    with open(os.path.join(os.path.dirname(__file__), "footer.json")) as a:
        return dict(settings360=json.load(a), now=datetime.now(),)
...
def data_registry():
    ...
    return render_template(
        "registry.html.j2", data=data, schema=schema, num_of_publishers=len(data)
    )
...
def show_publisher(prefix):
    ...
    return render_template("publisher.html.j2", publisher=publisher, files=files)
